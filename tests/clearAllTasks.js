...
const runClearAllTasksTests = async () => {
    ...
    try {
        ...
        const response = await fetch('http://127.0.0.1:2604/api/clearAllTasks', {
            method: 'POST',
            headers: {
        ...
        console.error(chalk.red('Failure test correctly failed for /clearAllTasks endpoint:', error.message));
    }
    ...
    try {
        ...
        if (data.error) {
            ...
            console.log(chalk.green('Test passed for /clearAllTasks endpoint'));
        }
    } catch (error) {
...
export default runClearAllTasksTests;
